...
export class ClaudeCodeCleaner {
  providerId = "claude-code";
  async resolveTargets(sourceFiles, homeDir) {
  ...
  // ── Helpers ────────────────────────────────────────────────────────────
  async addFileTarget(targets, path, description) {
    try {
  ...
  async addDirTarget(targets, path, description) {
    try {
      const s = await stat(path);
      if (s.isDirectory()) {
        targets.push({
  ...
  async addTodoTargets(targets, claudeDir, uuid) {
  ...
  removeFromSessionsIndex(indexPath, uuidsToRemove) {
    try {
      ...
      if (Array.isArray(index.entries)) {
        ...
        index.entries = index.entries.filter(
          (entry) => !uuidSet.has(entry.id ?? "") && !uuidSet.has(entry.sessionId ?? "")
        );
      }
      // Atomic write
