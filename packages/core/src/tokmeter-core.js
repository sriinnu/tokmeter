...
export class TokmeterCore {
   ...
   */
  async enrichCosts(records, warningScope, warnings) {
  ...
  async scanHistoricalRecords(providers, referenceTimestamp, warnings) {
  ...
  async scanTodayRecords(providers, referenceTimestamp, warnings) {
    const rawRecords = await this.scanRawRecords(providers, "today", warnings);
    return rawRecords.filter((record) => !isBeforeToday(record.timestamp, referenceTimestamp));
  }
  async scanRawRecords(providers, warningScope, warnings) {
    if (!this.skipPricing) {
      try {
  ...
  resolveTodayState(records, todayWarnings, isTodayOnlyScan) {
