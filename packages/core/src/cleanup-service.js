...
export class CleanupService {
  ...
  async preview(filter) {
    ...
    const sourceFilesByProvider = new Map();
   ...
   */
  detectPartialFiles(matchedRecords, allRecords) {
    // Build set of source files from matched records
    const matchedFiles = new Set();
    const matchedCountByFile = new Map();
    for (const r of matchedRecords) {
      if (!r.sourceFile) continue;
  ...
  buildProviderBreakdown(records, targets) {
    const map = new Map();
   ...
   */
  async createBackup(targets, filter, backupDir, projectNames = []) {
    const dir = backupDir || DEFAULT_BACKUP_DIR;
