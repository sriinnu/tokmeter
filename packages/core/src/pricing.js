 ...
 */
function staticPricing(modelId) {
...
export class PricingService {
  ...
  async getPricing(modelId) {
    ...
    if (this.registry) {
      ...
      try {
        const card = this.registry.model(modelId);
   ...
   */
  koshaFuzzySearch(modelId) {
    const normalized = modelId
      .toLowerCase()
      .replace(/-\d{8}$/, "")
      .replace(/-latest$/, "")
      .replace(/\./g, "-");
    const all = this.registry.models();
    const best = all
      .filter((m) => {
      ...
      .sort((a, b) => {
        ...
        const score = (base) => (base === normalized ? 2 : base.startsWith(normalized) ? 1 : 0);
   ...
   */
  roundPricing(p) {
    const r = (n) => Math.round(n * 1_000_000) / 1_000_000;
